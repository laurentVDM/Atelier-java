...
public class MultiThreadedStatsService {
    ...
    private Collection<Firstname> firstnames;
    private Collection<Integer> maxCounts;
    ...
    public MultiThreadedStatsService(Collection<Firstname> firstnames) {
    ...
    public void syncPrintFirstnameMaxCount() {
        ...
        maxCounts = new ArrayList<>();
        firstnames.forEach(firstname -> maxCounts.add(new CalculationResult(firstname.getName()).getResult()));
    ...
    public void asyncPrintFirstnameMaxCount() {
        ...
        for (Firstname firstname : firstnames) {
            CalculationWorker cw = new CalculationWorker(firstname.getName());
            synchronized (maxCounts) {
                maxCounts.add(cw.getResult());
            }
        }
    ...
    private int findMaxCountFromCalculationsResults() {
        ...
        return maxCounts.stream().reduce(0,Integer::max);
    }
    ...
    private List<String> findFirstnamesFromCalculationsResults(int occurrences) {
        ...
        return firstnames.stream().
            filter(f->f.getCounts().values().stream().reduce(Integer::sum).get().equals(occurrences))
    ...
    private class CalculationResult {
        ...
        public CalculationResult(String firstname) {
            this.firstname = firstname;
            this.result = getCount(firstname);
        }
    ...
    private class CalculationWorker extends Thread {
        ...
        public CalculationWorker(String firstname){
            this.firstname = firstname;
            this.start();
        }

        public void run(){
            this.result = new CalculationResult(this.firstname).getResult();
        }
