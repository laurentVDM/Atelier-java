...
public class StreamStatsService extends Thread {
    ...
    public List<String> getFirstnamesStartingWith(String prefix) {
        // TODO question 2
        return namesSupplier.get().map(Firstname::getName)
    ...
    public List<Firstname> getLongestFirstnames(int max) {
        ...
        return namesSupplier.get()
            //reverse order to get the longest first
    ...
    private City cityWithMaxFemaleNames() {
        ...
        return citiesSupplier.get()
            .max(Comparator.comparing(city -> city.getFirstnames().size())).get();
    }
    ...
    private List<String> top5NamesStartingWithPrefixLowestOccur(String prefix) {
        // TODO question 2
        return namesSupplier.get()
            .filter(f->f.getName().toLowerCase().startsWith(prefix.toLowerCase()))
            .sorted(Comparator.comparing(f -> f.getCounts().values().stream().reduce(0, Integer::sum)))
            .limit(5)
    ...
    private Map<Boolean, Long> numberOfCitiesWithMoreMThanF() {
        // TODO question 2
        return citiesSupplier.get()
            .collect(Collectors.partitioningBy(city ->city.getFirstnames()
                        .stream()
                        .filter(firstname -> firstname.
                            ...
                            .getFirstnames()
                            .stream()
                            .filter(firstname -> firstname.getGender().equals("f")).count()
                , Collectors.counting())
            );
    }
