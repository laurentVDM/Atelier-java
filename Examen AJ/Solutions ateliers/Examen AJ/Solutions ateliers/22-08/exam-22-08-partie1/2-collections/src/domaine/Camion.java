...
public class Camion  {
    ...
    public boolean ajouterTrajet(Trajet trajet){
        ...
        for (LocalDate date : trajets.keySet()) {

            if (date.equals(trajet.getDate())) return false;
            if (date.plusDays(1).equals(trajet.getDate())
                    && !trajets.get(date).getVilleArrivee().equals(trajet.getVilleDepart()))
                return false;
            if (date.minusDays(1).equals(trajet.getDate())
                    && !trajets.get(date).getVilleDepart().equals(trajet.getVilleArrivee()))
                return false;
        }
    ...
    public Trajet trouverTrajet(LocalDate dateTrajet){
        for (LocalDate date : trajets.keySet())
            if (date.equals(dateTrajet))
                return trajets.get(date);
    ...
    private Trajet rechercherTrajet(Caisse caisse) {
        Trajet trajet = null;
        for (LocalDate date : trajets.keySet()) {
            if (trajets.get(date).peutAjouter(caisse)
                && trajets.get(date).nbCaisses() < nbMaxCaisses
                && trajets.get(date).calculerPoidsTotal() + caisse.getPoids() <= chargeMaximale) {
                if (trajet == null || date.isBefore(trajet.getDate()))
                    trajet = trajets.get(date);
            }
        }
