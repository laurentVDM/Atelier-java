...
public class Camion  {
    ...
    public List<Trajet> trouverTrajetsAvecPlaceRestante(){
        return trajets.stream().filter(e -> e.nbCaisses() < nbMaxCaisses).collect(Collectors.toList());
    }
    ...
    public LocalDate trouverDateTrajet(String villeDepart, String villeDArrivee) {
        return trajets
                .stream()
                .filter(e -> e.getDate().isAfter(LocalDate.now()) && e.getVilleDepart().equals(villeDepart) && e.getVilleArrivee().equals(villeDArrivee))
                .map(Trajet::getDate)
                .reduce((a, b) -> {
                    if (a.isBefore(b))
                        return a;
