        ...
        for (Class c : getResourcesClasses()) {
            for (Method m : c.getDeclaredMethods()) {
                if (Modifier.isPublic(m.getModifiers()) && !Modifier.isStatic(m.getModifiers())) {
                    if (m.getAnnotation(Path.class).value().equals(request.getURL())) {
                        if (m.isAnnotationPresent(GET.class) && request.getMethod().equalsIgnoreCase("get")) return m;
                        if (m.isAnnotationPresent(POST.class) && request.getMethod().equalsIgnoreCase("post")) return m;
                    }
                }
            }
        }
