...
public class Server {
    ...
    Server(RequestFactory factory){
        this.requestFactory=factory;
    }

    public class ThreadListen extends Thread{
        Request request;
        ThreadListen(Request request){
    ...
    public void listenKeyboard() {
        ...
        while (true) {
            ...
            ThreadListen threadListen = new ThreadListen(request);
                ...
                if(!Modifier.isStatic(m.getModifiers())){
                    if(Modifier.isPublic(m.getModifiers())){
                        if(m.isAnnotationPresent(Path.class) &&
                            m.getAnnotation(Path.class).value().equals(request.getURL())){
