...
public class MoniteurImpl implements Moniteur {
	...
	/**
	 * ajoute un stage au moniteur. Ce stage est ajouté si le moniteur n'avait pas
	 * déjà un stage cette semaine-là et si ce stage nétait pas déjà dans sa liste
	 * de stages. Ce stage est stocké suivant son numéro de semaine. La méthode
	...
	public boolean ajouterStage(Stage stage) {
		...
		if (!stage.getSport().contientMoniteur(this))
			return false;
		if (!this.estLibre(stage.getNumeroDeSemaine())) return false;
