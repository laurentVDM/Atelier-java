...
public class Ferme {
  ...
  public VenteOeufs trouverVenteLaPlusGrande(Lot lot) {
    VenteOeufs venteOeufs= lot.getVentes().stream().max(Comparator.comparing(VenteOeufs::calculerPrixVente)).get();
  ...
  public List<VenteOeufs> trouverVentesPourNomAcheteur(String nomAcheteur) {
    List<VenteOeufs> venteOeufs= obtenirToutesLesVentes();

    // TODO Q3.2 : n'hésitez pas à partir du retour de la fonction obtenirToutesLesVentes()
    return venteOeufs.stream().filter(t->t.getNomAcheteur().equals(nomAcheteur)).sorted(Comparator.comparing(VenteOeufs::getDate)).collect(
        Collectors.toList());
  }
  ...
  public Map<String, Long> calculerOeufsVendusParNomAcheteur() {
    ...
    return obtenirToutesLesVentes().stream()
        .collect(Collectors.groupingBy(VenteOeufs::getNomAcheteur, Collectors.summingLong(VenteOeufs::getNombreOeufsVendus)));
  }
