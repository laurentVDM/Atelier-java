...
public class Ferme{
   ...
   */
  private static Set<VenteOeufs> ajouterVenteTrieePourAcheteur(Set<VenteOeufs> listePourAcheteur,
      VenteOeufs venteAajouter) {
    Set<VenteOeufs> nouvelleListePourAcheteurTriee =
        listePourAcheteur == null ? new HashSet<>() : listePourAcheteur;
    ...
    for(VenteOeufs venteOeufs : nouvelleListePourAcheteurTriee){
      LocalDate dateVenteActuel = venteOeufs.getDate();
      if (venteAajouter.getDate().isAfter(dateVenteActuel)) {
        nouvelleListePourAcheteurTriee.add(venteAajouter);
        break;
      } else if (venteAajouter.getDate().equals(dateVenteActuel)) {
        if (venteAajouter.getNombreOeufsVendus() > venteOeufs.getNombreOeufsVendus()) {
          nouvelleListePourAcheteurTriee.add(venteAajouter);
          break;
        }
      }
