...
public class StreamStatsService extends Thread {
    ...
    private City cityWithMaxFemaleNames() {
        // TODO question 2
        return citiesSupplier
            .get()
            .max(Comparator.comparing(city -> city.getFirstnames()
                .stream()
                .map(Firstname::getName)
    ...
    private List<String> top5NamesStartingWithPrefixLowestOccur(String prefix) {
        ...
        return namesSupplier
            .get()
            .filter(name->name.getName().toLowerCase().startsWith(prefix.toLowerCase()))
    ...
    private Map<Boolean, Long> numberOfCitiesWithMoreMThanF() {
        ...
        return citiesSupplier
            ...
            .collect(Collectors.partitioningBy(
                city -> city.getFirstnames()
                    .stream()
                    .filter(f->f.getGender().equals("m")).count()
                    >
                    city.getFirstnames()
                    .stream()
                    .filter(f->f.getGender().equals("f")).count()
                ,Collectors.counting()
            ));
    }
